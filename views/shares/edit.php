...
</div><?php $editID = new ShareModel();
            $rows = $editID->show();?>
<form enctype="multipart/form-data" class="cmxform" id="form1" method="POST" action="<?php echo  ROOT_URL;?>shares/edit" >
  <div class="container">
    <div class="row">
      <div class="col-sm-6 info-right-border">
        ...
        <div class="form-group">
          <p>
            ...
            <input id="title" class="form-control" name="title" minlength="2" value="<?php echo $rows['title'] ?>" type="text" required>
          </p>
        </div>
        <div class="form-group">
          ...
          <select id="Lloji-shpalljes" name="status" class="form-control">
            <?php
            if($rows["status"] === "Shitet"){
              echo '<option selected value="1">'. $rows["status"].'</option>';
              echo '<option value="2">Blihet</option>';
              echo '<option value="3">Qera</option>';
            }else if($rows['status'] == "Blihet"){
              echo '<option value="1">Shitet</option>';
              echo '<option selected value="2">'. $rows["status"].'</option>';
              echo '<option value="3">Qera</option>';
            }else if($rows['status'] == "Qera"){
              echo '<option value="1">Shitet</option>';
              echo '<option value="2">Blihet</option>';
              echo '<option selected value="3">'. $rows["status"].'</option>';
            }
            ?>
          </select>
        </div>
        <?php 
          // $typeID = $rows['typeID'];
          // $type = $rows['type'];
          //     echo '<option selected value="'.$typeID.'">'.$type.'</option>';
          //   for($i = 0; $i <= $rows['type']; $i++) :{
          //     echo '<option value="'.$typeID.'">'.$rows['type'].'</option>';
          //   }
          //  endfor; 
          ?> 
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="number" class="form-control" value="<?php echo $rows['surface'] ?>" id="Kuadratura-shpalljes" name="surface" required>
        </div>
        ...
        <div id="nrDhomav" hidden=""  style="display: block;">
          <div class="form-group nrDhomave" style="display: block;">
            ...
            <input type="number" class="form-control" value="<?php echo $rows['badroom'] ?>" id="Dhomat-shpalljes" name="badroom" required>
          </div>
        </div>
        ...
        <div class="form-group">
          <div class="row"> 
            <div class="col-md-9">
              ...
              <input type="number" class="form-control" value="<?php echo $rows['pricae'] ?>" id="Qmimi-shpalljes-label" name="price" required>
            </div>
            <div class="col-md-3">
              <label >&nbsp;</label>
        ...
        <div class="form-group">
          ...
          <textarea class="form-control form-control-textarea" id="Pershkrimi-shpalljes" name="description" rows="3" required><?php echo $rows['description'] ?>" </textarea>
        </div>
       ...
       <div class="col-sm-6">
        ...
        <div class="form-group">
          ...
          <input type="email" class="form-control" id="email-shpalljet" value="<?php echo $rows['email'] ?>" autocomplete="email" name="email" required>
        </div>

        <div class="form-group">
          <label for="Shteti-shpalljes">Shteti</label>
          <input type="text" id="" class="form-control" value="<?php echo $rows['state'] ?>">
          </br>
        ...
        <div class="form-group">
          ...
          <input id="tel-shpalles" type="text" value="<?php echo $rows['tel'] ?>" class="form-control" name="tel" required>
        </div> 
        ...
        <div class="form-group">
          ...
          <input type="file" name="filename[]" id="file-upload" class="inputfile" multiple/>
        </div>
      </div> 
    </div>
  </div> 
